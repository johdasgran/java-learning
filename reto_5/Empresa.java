...
public class Empresa {
	...
	public static ArrayList<Double> liquidarNominaEmp(ArrayList<Empleado> empleados) {
		...
		for (Empleado empleado : empleados) {
			...
			// System.out.println("El total v1 es:"+
			// total);---31.250-----122.500---4520--22.604
	...
	public static ArrayList<Double> liquidarParafiscales(ArrayList<Empleado> empleados) {
		...
		// Empresa.luquidarNominaEmp(empleados);
		...
		// Se podria pero para calcular este valor que pide el reto toca tomar
		// los decimales a las
		// horas extra xdd
		 ...
		 * 
		 * double pf = parafiscales.get(i)-106454; double total = pf*0.09;
		 * p.add(total);
		 * 
		 * }
		 */
	...
	public static ArrayList<Double> totalPagosOtorgadosAEmpleados(ArrayList<Empleado> empleados) {
		...
		ArrayList<Double> totalPagos = new ArrayList<Double>();

		ArrayList<Double> totalNomina = new ArrayList<Double>();
		ArrayList<Double> totalPrestaciones = new ArrayList<Double>();
		
		
		
		totalNomina = Empresa.liquidarNominaEmp(empleados);
		totalPrestaciones = Empresa.liquidarPrestacionesEmp(empleados);
		...
		for (int i = 0; i < empleados.size(); i++) {
			...
			total = totalNomina.get(i);
			//System.out.println("total NOmina " + total);
						
			prestaciones = totalPrestaciones.get(i);
			//System.out.println("total presta" + prestaciones);
			
			totalPagoEmp = total + prestaciones;
			
			totalPagos.add(totalPagoEmp);
